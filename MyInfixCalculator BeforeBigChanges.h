  ...
  private:
	...
	for (int i = 0; i<s.length(); i++){ //for every char in string s
		if (isDigit(s.at(i))){ //if char is a positive digit
			while ((isDigit(s.at(i)))||(s.at(i)=='.')){ //while we are still getting part of the number
				//i++;
			...
			if (s.at(i)=='-'){ //check if char denotes subtraction of negativity
				if ((s.at(i-1)=='(')||(operatorPrec(s.at(i-1)))){ //if char DOES denote negtivity
					temp += s.at(i);
					i++; //increment i
					while ((isDigit(s.at(i)))||(s.at(i)=='.')){ //get all digits in number
